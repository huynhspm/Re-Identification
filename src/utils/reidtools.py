## replace torchreid/reid/utils/reidtools.py in library torchreid
...
def visualize_ranked_results(distmat,
    ...
    def _cp_img_to(src, dst, rank, prefix, matched=False):
        ...
        if isinstance(src, (tuple, list)):
            if prefix == 'gallery':
                ...
                dst = osp.join(
                    dst, prefix + '_top' + str(rank).zfill(3)) + '_' + suffix
            else:
    ...
    for q_idx in range(num_q):
        ...
        text = qimg_path.split('/')[-2] + ' ' + str(qcamid)
        qimg_path_name = qimg_path[0] if isinstance(qimg_path,
                                                    (tuple,
                                                     list)) else qimg_path
        ...
        if data_type == 'image':
            ...
            grid_img = 255 * np.ones(
                (height + HEIGHT_TEXT, num_cols * width + topk * GRID_SPACING +
                 QUERY_EXTRA_SPACING, 3),
                dtype=np.uint8)
            grid_img[:height, :width, :] = qimg
            cv2.putText(grid_img, text, (0, height + HEIGHT_TEXT - 2), FONT,
                        FONT_SCALE, (0, 0, 0), FONT_THICKNESS, cv2.LINE_AA)
        else:
            qdir = osp.join(save_dir,
                            osp.basename(osp.splitext(qimg_path_name)[0]))
        ...
        for g_idx in indices[q_idx, :]:
            ...
            text = gimg_path.split('/')[-2] + ' ' + str(gcamid)
            ...
            if not invalid:
                ...
                if data_type == 'image':
                    ...
                    cv2.putText(grid_img, text,
                                (start, height + HEIGHT_TEXT - 2), FONT,
